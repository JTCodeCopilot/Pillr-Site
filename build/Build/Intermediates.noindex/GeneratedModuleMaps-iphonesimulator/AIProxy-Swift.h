...
#ifndef AIPROXY_SWIFT_H
#define AIPROXY_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="AIProxy",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// <h2>Implementor’s note, and a gotcha</h2>
...
/// )
///
/// \endcodeIn a perfect world, this would be all that is required of you. In fact, it is all that is required to protect requests made
/// with <code>await mySession.data(for:)</code>, because Foundation calls <code>urlSession:didReceiveChallenge:</code>
/// internally. However, <code>await mySession.bytes(for:)</code> is not protected, which is rather odd. As a workaround,
...
/// )
///
/// \endcodeIf you encounter other calls in the wild that do not invoke <code>urlSession:didReceiveChallenge:</code> on this class,
/// please report them to me.
SWIFT_CLASS("_TtC7AIProxy33AIProxyCertificatePinningDelegate")
@interface AIProxyCertificatePinningDelegate : NSObject <NSURLSessionTaskDelegate>
